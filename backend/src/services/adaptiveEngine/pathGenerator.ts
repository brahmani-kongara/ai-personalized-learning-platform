import { PathNode, SequencedLearningPath, TopicData } from './types.js';
...
export function generateLearningPath(
  ...
  learnerId: string,
  topics: TopicData[],
  topicMasteryMap: Record<string, number> = {}, // topicId -> masteryScore (0 - 100)
  learnerBaselineSkill: number = 3
): SequencedLearningPath {
  ...
  const topicById = new Map<string, TopicData>();
  ...
  const pathNodes: PathNode[] = sortedTopicIds.map((topicId, index) => {
    const topic = topicById.get(topicId)!;
    const masteryScore = Math.min(100, Math.max(0, topicMasteryMap[topicId] || 0));

    // Check if all prerequisites meet threshold
    const prerequisitesCompleted = topic.prerequisiteIds.every(
      (prereqId) => (topicMasteryMap[prereqId] || 0) >= MASTERY_THRESHOLD
    );
