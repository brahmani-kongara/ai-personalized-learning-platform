 ...
 */
export const DEFAULT_STYLE_FORMAT_MAP: Record<LearningStyle, DeliveryFormat> = {
  VISUAL: 'VIDEO',
  AUDITORY: 'VIDEO',
  READING: 'TEXT',
 ...
 */
export const ALTERNATE_FORMAT_MAP: Record<DeliveryFormat, DeliveryFormat[]> = {
...
export function calculateNextDifficulty(
  ...
  const defaultFormat = DEFAULT_STYLE_FORMAT_MAP[preferredStyle] || 'VIDEO';
  ...
  for (let i = history.length - 1; i >= 0; i--) {
    ...
    if (record.isCorrect) {
      if (consecutiveWrong > 0) break; // Break streak on state change
      consecutiveCorrect++;
    } else {
      if (consecutiveCorrect > 0) break; // Break streak on state change
      consecutiveWrong++;
    }
  }
  ...
  if (consecutiveWrong >= 2) {
    ...
    const alternateOptions = ALTERNATE_FORMAT_MAP[lastAttempt.format] || ['INTERACTIVE', 'VIDEO'];
 ...
 */
export function selectNextContentItem(
  availableItems: ContentItemData[],
  ...
  let bestItem: ContentItemData | null = null;
